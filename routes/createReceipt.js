...
router.post('/uploadReceipt', function(req, res, next) {
  ...
  req.on('end', () => { // when data (body component of http request) is collected
      ...
      var params = 'email=' + req.session.email + '&hashedPassword=' + req.session.password + '&institutionName=' + loginData.institutionName + '&creatorUserEmail=' +req.session.email +'&documentItems=[';
      ...
      params=params+']&paymentMethodID='+ loginData.paymentMethodID;
      console.log(params);
      var options = {
          hostname: rootPath,
          port: 80,
          path:'/Document/UploadReceipt.php',
          method: 'POST',
          headers: {
