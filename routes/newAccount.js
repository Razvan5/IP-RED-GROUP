...
router.post('/', function (req, res, next) {
  ...
  req.on('end', () => {              // when data (body component of http request) is collected
    ...
    // our path with parameters: email , hashedPassword , firstName and lastName
    var params = 'email=' + loginData.email + '&hashedPassword=' + loginData.password + '&firstName=' + loginData.firstname + '&lastName=' + loginData.lastname;
    console.log(params);
    var options = {
      hostname: rootPath,
      port: 80,
      path: '/Account/Create.php',
      method: 'POST',
      headers: {
